...
read -rp "你需要TAG标签吗(Y/N): " chrony_install
    ...
    [yY][eE][sS] | [yY])
        ...
        docker run --name nginx-mtproxy -d -e tag="$tag" -e secret="$secret" -e domain="$domain" -p 80:80 -p $port:$port
        ;;
    *)
...
echo -e "正在安装nginx-mtproxy... "
docker load -i ellermister_nginx-mtproxy.tar
docker run --name nginx-mtproxy -d -e secret="$secret" -e domain="$domain" -p 80:80 -p $port:$port
        ;;
    esac
